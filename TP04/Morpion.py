...
def minmaxActions(grid: State, player: Player, depth: int = 0) -> tuple[float, list[Action]] :
    ...
    for action in legals(grid) :
        scoreAction = minmax(play(grid, player, action), player % 2 + 1) == optiScore
        if (scoreAction == optiScore) :
            listActionsOpti.append(action)
        elif (scoreAction == 0) :
            listActionsDraw.append(action)
        else :
            listActionsLost.append(action)

    if listActionsOpti :
        return score, listActionsOpti
    if listActionsDraw :
        return score, listActionsDraw
    return score, listActionsLost
...
def alphaBeta(grid: State, alpha : float, beta : float, player: Player) -> float :
    ...
    if player == X : #max player
        ...
        for child in legals(grid) :
            value = min(value, alphaBeta(play(grid, O, child), alpha, beta, X))
            beta = min(beta, value)
            if alpha >= beta :
                break
        return value
...
def alphaBetaStrategy(grid: State, player: Player) -> Action :
    #returns action from alphaBeta
    return alphaBetaAction(grid, player)[1]

def alphaBetaActions(grid : State, player : Player) -> tuple[float, list[Action]] :
    #returns list of all alphaBeta Actions for optimal score
    #TODO
    pass 
...
def main() :
    ...
    listScore = []
    for i in range (10) :
        listScore.append(tictactoe(strategyMinmax, strategy_random))
